...
require_root
for v in P1_RATIO ASM_DISK_NUM NODE1_HOSTNAME NODE2_HOSTNAME; do
  require_var "${v}"
done
...
# In SEHA cluster mode, node2 is responsible for initial partitioning; the
# shared disk writes propagate to node1.
partition_here="false"
if [[ "${current_host}" == "${NODE2_HOSTNAME}" ]]; then
  partition_here="true"
fi
