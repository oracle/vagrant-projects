...
# patched by different mechanisms: the Grid home in place at install time by
# 'gridSetup.sh -applyRU', the RDBMS home afterwards by opatchauto.
#
# Both Oracle homes on the FPP server belong to the grid user — the RDBMS home
# here exists to host the GIMR, not a user database — so every staged tree and
# OPatch install below is owned by grid:oinstall rather than oracle:oinstall.
#------------------------------------------------------------------------------
...
# Stage the same GI RU zip for the RDBMS home, leaving GI_RU_PATCH_DIR pointing
# at the tree opatchauto should apply. 07_extract_gi.sh has normally staged the
# same zip already; stage_ru_patch is idempotent, so this just re-points the
# tree at grid and hands the location back.
#
# Unused on 19c, which has no separate RDBMS home: its GIMR is configured
# inline by gridSetup.sh, so the GI RU is the only patch that applies there.
prepare_db_ru_patch() {
  local oracle_home="$1"
  local patch_dir patch_id
