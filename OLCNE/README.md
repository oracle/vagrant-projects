# Oracle Linux Cloud Native Environment on Oracle Linux 7

This Vagrantfile will deploy and configure the following components:

- One or more master nodes (1 by default, 3 in HA mode)
- One or more worker nodes (2 by default)
- An optional operator node for the Oracle Linux Cloud Native Environment
  Platform API Server and Platform CLI tool (installed by default on the first
  master node if an operator node is not enabled)
...
will not work in a VirtualBox guest unless nested virtualization support is
enabled.
...
- `CPUS` (default: `4`): all VMs are provisioned with 4 vCPUs.
- `MEMORY` (default: `5120`): all VMs are provisioned with 5GB memory.
