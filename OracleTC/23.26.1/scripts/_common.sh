...
#
# _common.sh
#   Shared helpers for all provisioning scripts in the Oracle True Cache project.
#   Must be sourced, not executed:  . /vagrant/scripts/_common.sh
#------------------------------------------------------------------------------

# Re-entrancy guard
if [[ -n "${__TC_COMMON_SH_LOADED:-}" ]]; then
  return 0
fi
__TC_COMMON_SH_LOADED=1
...
: "${TC_SETUP_ENV_FILE:=/etc/opt/oracle-tc/setup.env}"
if [[ -r "${TC_SETUP_ENV_FILE}" ]]; then
  # setup.env is trusted: written by this project's setup.sh
  # shellcheck disable=SC1090
  . "${TC_SETUP_ENV_FILE}"
elif [[ -e "${TC_SETUP_ENV_FILE}" ]]; then
  log_error "setup env '${TC_SETUP_ENV_FILE}' is not readable by user '$(id -un)'"
  exit 1
fi
...
# (not opatchauto). The patch is applied before the True Cache instance is
# created so no instance is ever running when opatch runs.
#------------------------------------------------------------------------------
