...
| --- | --- | --- |
| `ORCL_software/LINUX.X64_2326100_grid_home.zip` | Yes | Host-side filename/presence checks, manifest entry check, host-side SHA-256 verification |
| `ORCL_software/LINUX.X64_2326100_db_home.zip` | Yes | Host-side filename/presence checks, manifest entry check, host-side SHA-256 verification |
| `ORCL_software/p6880880_230000_Linux-x86-64.zip` (OPatch) | Only if applying an RU | Presence + manifest entry + host-side SHA-256 verification |
| `ORCL_software/p<bug-number>_230000_Linux-x86-64.zip` (GI RU) | Only if applying an RU | Presence + manifest entry + host-side SHA-256 verification |
...
| 07 | [`scripts/07_extract_gi.sh`](scripts/07_extract_gi.sh) | Unpacks the Grid home, patches Oracle's bundled `sshUserSetup.sh` to use 2048-bit RSA keys on OL9, and stages the RU when one is configured |
