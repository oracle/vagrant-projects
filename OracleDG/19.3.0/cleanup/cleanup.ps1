...
#
# cleanup.ps1
#   Windows (PowerShell) port of cleanup.sh. Tears down the Data Guard lab and
#   removes the per-node u01 disks (primary_u01.vdi / standby_u01.vdi) and the
#   oradata disks (<role>_oradata_disk<i>.vdi) that `vagrant destroy` leaves
#   behind on VirtualBox.
#
# Usage:
...
if (-not $Force) {
    ...
    if ($Provider -eq 'virtualbox') {
        Write-Host "  2. delete per-node u01 disks ($(Split-Path -Leaf $U01H1), $(Split-Path -Leaf $U01H2))"
        Write-Host "  3. delete $OradataNumInt oradata disk(s) per node (primary/standby)"
    }
    Write-Host ""
...
switch ($Provider) {
    ...
    'virtualbox' {
        ...
        Write-Host '=== removing VirtualBox per-node u01 disks ==='
        foreach ($u01 in $U01H1, $U01H2) {
            $p = [System.IO.Path]::GetFullPath((Join-Path '.' $u01))
        ...
        foreach ($role in 'primary', 'standby') {
            for ($i = 0; $i -lt $OradataNumInt; $i++) {
                $p = [System.IO.Path]::GetFullPath((Join-Path $dir "${role}_oradata_disk$i.vdi"))
                Invoke-VBoxCloseAndDelete -Path $p -VBoxManage $vbm
            }
        }
    }
