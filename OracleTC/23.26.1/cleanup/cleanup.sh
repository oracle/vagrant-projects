...
#
# cleanup.sh
#   Tear down the True Cache lab and remove the disks that `vagrant destroy`
#   leaves behind:
#     * VirtualBox — the u01 disk (primary_u01.vdi) is intentionally kept across
#       destroy/up so a re-provision is fast, and the oradata disks
#       (primary_oradata_disk<i>.vdi) are separate media. Full cleanup deletes
#       both.
...
# VirtualBox u01 disk (default to the Vagrantfile's own default).
...
EOF
  if [[ "$PROVIDER" == "virtualbox" ]]; then
    echo "  2. delete the u01 disk (${U01_H1##*/})"
    echo "  3. delete ${ORADATA_NUM} oradata disk(s)"
  else
    echo "  2. sweep leftover per-domain volumes for ${PREFIX}host1"
  fi
  echo ""
...
# u01 and oradata volumes all share that stem.
...
case "$PROVIDER" in
  libvirt)
    pools=$(printf '%s\n' "$POOL_H1" "$POOL_ORADATA" | awk 'NF' | sort -u)
