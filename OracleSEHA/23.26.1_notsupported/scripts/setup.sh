...
if [[ "${ORESTART}" != "false" || "${DB_TYPE}" != "SEHA" ]]; then
  echo "ERROR: SEHA requires ORESTART=false and DB_TYPE=SEHA" >&2
  exit 1
fi

# SEHA is a Standard Edition 2 feature, so an EE home cannot host a SEHA
# database. Checked here, before any provisioning: the alternative is failing
# ~30 minutes in, at runInstaller, with INS-35464.
if [[ "${DB_INSTALL_EDITION}" != "SE2" ]]; then
  echo "ERROR: SEHA requires a Standard Edition 2 database home, but DB_INSTALL_EDITION=${DB_INSTALL_EDITION}." >&2
  echo "ERROR: the 23.26.1 (Oracle AI Database 26ai) Linux x86-64 db_home.zip ships Enterprise Edition only:" >&2
  echo "ERROR:   runInstaller rejects installEdition=SE2 with [INS-35464] 'The installer does not" >&2
  echo "ERROR:   support installing the Standard Edition 2 of Oracle Database.'" >&2
  echo "ERROR: This media therefore cannot build a SEHA lab. Use the 19.3.0 or 21.3.0 project, or" >&2
  echo "ERROR: supply an SE2 shiphome for this release and set DB_INSTALL_EDITION=SE2 in Vagrantfile." >&2
  exit 1
fi
...
SETUP_ENV_DIR="/etc/opt/oracle-seha"
...
( umask 077
  {
    ...
    write_env_export DB_INSTALL_EDITION "${DB_INSTALL_EDITION}"
    printf '\n'
...
# -------------------- node1 drives Grid Infrastructure and database setup ----
...
log_success "SEHA provisioning complete on ${current_host}"
